...
class GitRepository(object):
    ...
    def __init__(self, path, force=False):
        ...
        if cf and os.path.exists(cf):
            ...
            raise Exception("Configration file missing")
...
def repo_file(repo, *path, mkdir=False):
    """Same as repo_path, but create dirname(*path) if absent.
        For example, repo_file(r, \"dirA\", \"dirB\", \"foo\") will create .git/dirA/dirB."""
...
def repo_create(path):
    ...
    # First, we make sure the path either does not exist or is
    # an empty dir.
    ...
    with open(repo_file(repo, "description"), "w") as f:
        f.write("Unnamed repository; edit this file 'discription' to name the repository.\n")
...
argsp = argsubparsers.add_parser("hash-object",
                                 help="Comput object ID and optionally creates a blob from a file")
...
def kvlm_parse(raw, start=0, dct=None):
    """
    kvlm means Key-Value Lists with Message.
    """
    if not dct:
        dct = collections.OrderedDict()
        # You CANNOT declare the argument as dct=OrderedDict().
        # If you do so, everytime the function is called, the same dicts are made endlessly.
    ...
    # (訳) value の終わりを探す．ひと続きの複数行はスペースから始まるので，
    # 次が空白じゃない\nを探す
    end = start
    while True:
        end = raw.find(b'\n', end+1)
...
def kvlm_serialize(kvlm):
    ...
    for k in kvlm.keys():
        ...
        # (訳) ファイル内容そのものは飛ばす．(ファイル内容そのものはb''のキーに登録されている)
        if k == b'':
            continue
        val = kvlm[k]
        # Normalize to a list
...
def tree_parse_one(raw, start=0):
    ...
    sha = hex(
        int.from_bytes(
            raw[y+1:y+21], "big"))[2:]  # hex() add 0x in front.
                                            # we don't want that.
...
def cmd_checkout(args):
    ...
    if os.path.exists(args.path):
        if not os.path.isdir(args.path):
            raise Exception("Not a directoy {0}".format(args.path))
...
def object_resolve(repo, name):
    ...
    if hashRE.match(name):
        if len(name) == 40:
            ...
            # This is a small hash 4 seems to be minimal length
            # for git to consider something a short hash.
            # This limit is documented in man git-rev-parse
            name = name.lower()
            # Remember there is a dir
            # which is named first two char of the hash.
            prefix = name[0:2]
